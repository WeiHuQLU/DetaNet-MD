...
"""DetaNet graph neural network model"""

'''Irreducible Representations(irreps) of vectorial and tensorial feature
    In DetaNet, vectors and tensors are represented by Irreducible Representation.
    
    The irrep of a feature in terms of rotation degree and parity.
...
class DetaNet(nn.Module):
    ...
    device: device type, either torch.device('cuda') represents gpu or torch.device('cpu')  represents cpu
        ...
        if out_type == '2_tensor' or out_type=='stress' or out_type=='stress_2':
    ...
    def cal_stress_tensor(self,cell, pos, outs, outt, batch):
        sa, sb = torch.split(outs, dim=-1, split_size_or_sections=[1, 1])
        centers = torch.sum(cell, dim=1) / 2
        volume = torch.abs(torch.linalg.det(cell))
        if batch is not None:
            ra=pos-centers[batch]
            volume=volume[batch, None, None]
        else:
            ra=pos-centers
        ta=o3.spherical_harmonics(l=self.irreps_out,x=ra,normalize=False)*sa
        stress_whole=self.ct.to_cartesian(torch.concat(tensors=(sb,outt+ta),dim=-1))
        return stress_whole/volume
    ...
    def grad_polarzability(self, polars, pos):
        '''
        Calculating the derivative of the polarizability with respect to the coordinates
    ...
    def grad_stress(self, energy, strain, box, batch):
        stress = -grad([torch.sum(energy)], [strain], create_graph=True)[0]
        volume = torch.abs(torch.linalg.det(box))
        if batch is not None:
            volume = volume[batch, None, None]
        return stress.reshape(-1,3,3)/volume
    ...
    def forward(self,
        ...
        edge_vector: inter-atomic vector with edge, FloatTensor of shape [num_edge,3],
        which is provides for crystal and other periodicity molecule training and predicting.
        input complete periodicity edge_index and edge_vec from pymatgen or other material software.
        and set position is None
        ...
        if self.grad is not None:
            if not pos.requires_grad:
                pos = pos.requires_grad_()  # TorchScript 兼容写法
        ...
        sh = o3.spherical_harmonics(l=self.irreps_sh, x=edge_vec/(edge_weight.view(-1,1)), normalize=True, normalization="component")
        #Radial basis are generated from distances.
        rbf = self.Radial(edge_weight)

        #via interaction layers
        for block in self.blocks:
            S,T=block(S=S,T=T,sh=sh,rbf=rbf,index=edge_index)
        #for blk in self.blocks:
            #S, T = blk(S=S, T=T, sh=sh, rbf=rbf, index=edge_index)
        ...
        if self.ref is not None:
            out=out+(self.ref.to(out.device)[z]).reshape(-1,1)

        #Summation of atomic properties to obtain molecular properties
        if self.summation:
            assert self.mean is False
            if batch is not None:
                out = scatter(src=out, index=batch, dim=0)
            else:
                out = torch.sum(input=out, dim=0)

        elif self.mean is not False:
            assert self.summation is False
            if batch is not None:
                out = scatter(src=out, index=batch, dim=0, reduce= 'mean')
            else:
                out = torch.mean(input= out, dim=0)
        ...
        if self.grad=='force':
            ...
            energy=out
            force=self.grad_force(energy=energy,pos=pos)
            out=(energy,force)
        ...
        if self.scale is not None and self.grad != 'ef':
            out=out*self.scale
        return out
