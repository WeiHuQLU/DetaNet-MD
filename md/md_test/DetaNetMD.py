...
import os
import sys
sys.path.append('/home/huwei19/JSJ/detanet-md/')
...
from md_test.DetaNetCalculator import DetaNetCalculator
from schnetpack.md.simulation_hooks import NHCThermostat,LangevinThermostat,PILELocalThermostat,BerendsenThermostat
...
class  MolecularDynamics :
    def __init__(self,molecule,model_files,md_params,md_workdir,md_device='cuda'):
        ...
        md_initializer = MaxwellBoltzmannInit(self.md_params["system_temperature"],
                                              remove_center_of_mass=True,
                                              remove_translation=True, remove_rotation=True)
        ...
        cutoff = 7.0  # Angstrom (units used in model)
        cutoff_shell = 2.0  # Angstrom

        #initialize neighbor list for MD using the ASENeighborlist as basis  ,'polarizability', 'dipole_moment'
        ...
        self.md_calculator = DetaNetCalculator(
            ...
            energy_key=None,
            required_properties=['forces','dipole_moment'],
        ...
        #Berendsen = BerendsenThermostat(bath_temperature, time_constant)
        self.simulation_hooks = [Nose_Hover]

 
    def run(self,MDRestart):
        # Gnerate a directory of not present
        
        if not os.path.exists(self.md_workdir):
            os.makedirs(self.md_workdir, exist_ok=True)    
        # Path to database
        ...
        # Set up data streams to store positions, momenta and the energy   ,properties.polarizability, properties.dipole_moment
        data_streams = [
            callback_hooks.MoleculeStream(store_velocities=True),
            callback_hooks.PropertyStream(
                target_properties=[properties.forces, properties.dipole_moment
                              ]),
        ]
...
class RPMolecularDynamics:
     ...
     def run(self, MDRestart=False):
         ...
         # Path to database
         log_file = os.path.join(self.rpmd_workdir, "simulation.hdf5")

         # Size of the buffer
         buffer_size = 100

         # Set up data streams to store positions, momenta and the energy properties.polarizability    ,properties.polarizability
         data_streams = [
             callback_hooks.MoleculeStream(store_velocities=True),
             callback_hooks.PropertyStream(
                 target_properties=[properties.forces, properties.dipole_moment
